...
totalj=0
deg="\$(grep -n "<dict/>" \$HOME/Documents/FlexC/patches.plist | cut -f1 -d:)"
if [ "\$deg" != "" ]; then
  IFS=\$'\n' read -rd '' -a ar <<< "\$deg"
  for dsc in "\${ar[@]}"; do
    sed -e "\${dsc}d" \$HOME/Documents/FlexC/patches.plist >/tmp/dkt.txt
    rm \$HOME/Documents/FlexC/patches.plist
    mv /tmp/dkt.txt \$HOME/Documents/FlexC/patches.plist
  done
fi
for (( x=1; x<=\$ef1; x++ )); do
