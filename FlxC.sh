#!/bin/bash
if [[ $EUID -ne 0 ]]; then echo -en "\e[1;49;91mFlxC must be run as root! (su)\e[0m\n" && exit; fi
plists=( "/private/var/mobile/Library/Application Support/Flex3/patches.plist" "/var/mobile/Documents/Flex/patches.plist" "/private/var/mobile/Library/UserConfigurationProfiles/PublicInfo/Flex3Patches.plist" "/private/var/mobile/Library/ConfigurationProfiles/PublicInfo/Flex3Patches.plist" "/private/var/mobile/Library/Carrier Bundles/Overlay/Flex3Patches.plist" "/private/var/mobile/Library/VoiceServices/Assets/Flex3Patches.plist" "/private/var/mobile/Media/iTunes_Control/Artwork/Flex3Patches.plist" )
...
if [ ! -d "/var/mobile/Documents/Flex" ]; then echo -en "\e[1;49;91mNo Flex Folder Found\e[0m @/var/mobile/Documents/Flex\n" && exit; fi
if [ ! -d "/var/mobile/Documents/Flex/Converted" ]; then mkdir "/var/mobile/Documents/Flex/Converted"; fi
if [ "${plist_path}" == "" ]; then echo -en "\e[1;49;91mCould not find patches.plist anywhere!\e[0m\n" && exit; fi
hasXML="$(command -v xmllint)"
if [ "$hasXML" == "" ]; then echo -en "\e[1;49;91mxmllint not found!\e[0m Install 'xml2' from cydia.\n" && exit; fi
...
if [ "$*" == "" ]; then
  ...
  select hu in "${qko[@]}" "Help" "Quit"; do
    if [[ -n "$hu" ]]; then
      if [ "$hu" == "Help" ]; then
        echo -en "\e[5;40;97mFlxC -> Flex2Hook Converter\e[0m\nCurrent Version: 1.1\n\n\e[1;49;96mUsage:\e[0m\n\e[1;49;39mFlxC\e[0m (Shows Menu to select Patch)\n\e[1;49;39mFlxC [ Patch Name ]\e[0m\n\n\e[1;49;39mIF\e[0m you find any \e[1;49;91mbugs\e[0m then contact @\e[1;49;32mTheArmKing\e[0m on \e[1;49;94mDiscord\e[0m: \e[1;49;39mdiscord.gg/85jceTE\e[0m\n"
      elif [ "$hu" == "Quit" ]; then
        exit
      else
        pewt="$hu"
...
echo -en "// Generated using FlxC.sh! If you find any bugs then contact @TheArmKing on Discord: discord.gg/85jceTE\n\n#include <substrate.h>\n" >>/var/mobile/Documents/Flex/tweak.txt
cles=()
...
hasPB="$(command -v pbcopy)"
if [ "$hasPB" != "" ]; then
  pbcopy < /var/mobile/Documents/Flex/tweak.txt
  echo -en "\n\e[1;49;92mCopied to Clipboard!\e[0m\n\n"
else
  echo -en "\n\e[1;49;91mpbcopy not found!\e[0m You can get it from https://github.com/TheArmKing/pbcopy-ios if you want the output to be copied to your clipboard.\n\n"
fi
while (( !xss )); do
  ...
  if [ "$choi" == "y" ]; then
    ...
    ew=1
    while (( !fdg )); do
      if [ -f "/var/mobile/Documents/Flex/Converted/${idy}_$ew.xm" ]; then
        (( ew = ew + 1 ))
      else
        cp /var/mobile/Documents/Flex/tweak.txt "/var/mobile/Documents/Flex/Converted/${idy}_$ew.xm"
        fdg=1
      fi
    done
    echo -en "\e[1;49;92mSaved\e[0m to /var/mobile/Documents/Flex/Converted/${idy}_$ew.xm\n"
    xss=1
  elif [ "$choi" == "n" ]; then
    xss=1
  else
